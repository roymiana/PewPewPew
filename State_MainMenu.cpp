...
void State_MainMenu::OnCreate(){
	...
	//background
	m_background.loadFromFile("introback.png");
	...
	//how to play
	m_tutexture.loadFromFile("howtoplay.png");
	m_tutoSprite.setTexture(m_tutexture);
	m_tutoSprite.setOrigin(0,0);

	m_tutoSprite.setPosition(0, 0);
	...
	//music
	theme1.openFromFile("title.flac");
	theme1.play();
...
void State_MainMenu::MouseClick(EventDetails* l_details){
	...
	for(int i = 0; i < 3; ++i){
		...
		{
			if(i == 0){
				m_stateMgr->SwitchTos(StateType::PreGame);
			} else if(i == 1){
				m_stateMgr->SwitchTos(StateType::Credits);
			} else if(i == 2){
