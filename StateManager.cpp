	...
	: m_shared(l_shared)
{
	...
	RegisterState<PreGame>(StateType::PreGame);
	RegisterState<State_Game>(StateType::Game);
	RegisterState<State_Paused>(StateType::Paused);
	RegisterState<State_GameOver>(StateType::GameOver);
	RegisterState<Credits>(StateType::Credits);
}
...
void StateManager::SwitchTos(const StateType& l_type){
	m_shared->m_eventManager->SetCurrentStates(l_type);
	for (auto itr = m_states.begin();
		itr != m_states.end(); ++itr)
	{
		if (itr->first == l_type){
